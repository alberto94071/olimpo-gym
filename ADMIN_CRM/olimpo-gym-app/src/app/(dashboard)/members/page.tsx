...
import { MemberClientFilters } from "@/components/members/MemberClientFilters";
...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ q?: string, status?: string, gymId?: string, limit?: string, page?: string }> }) {
  ...
  const { data, totalCount } = await getMembers({ searchQuery: q, statusFilter: status, gymIdFilter: gymId, limit, offset });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-olimpo-text-muted mt-1">
            Gestiona los clientes inscritos en la sede.
          </p>
        </div>
        <Link 
          href="/members/new"
      ...
      <MemberClientFilters userRole={currentUser.role} gyms={allGyms} />
